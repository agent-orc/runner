...
> **Status: core complete, pre-1.0.** Extracted and generalized from **Agent Studio**, a production multi-agent orchestrator that has processed hundreds of millions of tokens through these CLIs. The spawn engine, descriptor-driven CLI catalog, event contract, outcome model, quota module, metrics recorder and optional rendering package are implemented and tested (566 tests, CI on Windows + Linux). BenchmarkDotNet micro-benchmarks are available as an optional manual run. The public API may still shift before 1.0 — pin a version and watch releases.
...
- ✅ Pluggable process spawner (`CliOptions.Spawner` / `ICliProcessSpawner`) — use `CliProcessSpawner.Decorate` to retain the default Windows handle scrub while adding host policy, or inject a custom launcher (e.g. a Windows PTY).
